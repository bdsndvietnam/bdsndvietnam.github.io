...
class baogia
{
	...
	{
		mysql_query( "DELETE FROM yeucausim WHERE simid='".$id."'" );
	}
	...
	public function xoaall( $arr )
	{
		...
		{
			mysql_query( "DELETE FROM yeucausim WHERE id='".$value."'" );
		}
	}
...
$sql = query( "SELECT * FROM yeucausim ".$where." ".$_GET['post']['oder'].( " ".$limit ) );
$stv->assign( "mysql", urlencode( "SELECT * FROM yeucausim ".$where." ".$_GET['post']['oder']."" ) );

if ( 0 < $_GET['delid'] )
{
	query( "DELETE FROM yeucausim WHERE id='".$_GET['delid']."'" );
}

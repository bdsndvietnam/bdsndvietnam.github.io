...
else
{
	...
	if ( $_GET['del'] )
	{
		$q0 = query( "SELECT * FROM menu WHERE mgr=".$_GET['del'] );
		if ( mysql_num_rows( $q0 ) < 1 )
		{
			query( "DELETE FROM menu WHERE mid=".$_GET['del'] );
		}
	}
	$masp = 20;
	$page = $_GET['page'];
	...
	{
		$q = query( "SELECT * FROM menu WHERE menu!='' ORDER BY moder limit ".$bg.",{$masp}" );
		$dem = mysql_num_rows( query( "SELECT * FROM menu WHERE menu!='' ORDER BY moder" ) );
	}
	if ( $page == 1 )
	{
		$i = 0;

...
$sql = mysql_query( "SELECT my_order.id AS id, my_order.ngaygio AS ngaygio, my_order.trangthai AS trangthai, my_order.hovaten AS hovaten, my_order.diachi AS diachi, my_order.dienthoai AS dienthoai, my_order.sosim AS sosim, my_order.gianhap AS gianhap,my_order.giatien AS giatien, my_order.city AS city, my_order.daily AS daily, thanhvien.mobile AS mobiledl, my_order.dailyid AS dlid FROM my_order LEFT JOIN thanhvien ON my_order.dailyid = thanhvien.id  ".$where." order by id desc limit {$bg},{$masp}" );

...
class baogia
{
	...
	{
		mysql_query( "DELETE FROM simmoi WHERE simid='".$simid."'" );
	}

	public function dauso( )
	{
		$sql = query( "SELECT left(sim2,4) AS dau FROM simmoi GROUP BY left(sim2,4) ORDER by left(sim2,4) DESC" );
		while ( $row = fecth( $sql ) )
		{
			$arr[] = $row['dau'];
		}
	...
	public function xoaall( $arr )
	{
		...
		{
			mysql_query( "DELETE FROM simmoi WHERE simid='".$value."'" );
		}
	}

	public function suall( $arr1, $arr2 )
	{
		if ( is_array( $arr1 ) )
		{
			foreach ( $arr1 as $k => $v )
...
$stv = new Smarty( );
$q2 = mysql_query( "SELECT * FROM thanhvien WHERE live > 0 order by hovaten asc" );
...
if ( is_array( $_GET['n'] ) )
{
	foreach ( $GLOBALS['_GET']['n'] as $v )
	{
		$vv .= $v;
	}
	$where .= " AND sim2 NOT rlike'[".$vv."]'";
}
if ( $_GET['post']['dauso'] != 0 )
{
	$where .= " AND left(sim2,4)='".$_GET['post']['dauso']."'";
}
if ( $_GET['post']['daily'] != 0 )
{
	$where .= " AND simdl='".$_GET['post']['daily']."'";
}
if ( 1 < strlen( $_GET[post][loaisim] ) )
{
...
$sql = query( "SELECT * FROM simmoi ".$where." ".$_GET['post']['oder'].( " ".$limit ) );
$dem = mysql_num_rows( query( "SELECT * FROM simmoi ".$where." ".$_GET['post']['oder']."" ) );
$stv->assign( "mysql", urlencode( "SELECT * FROM simmoi ".$where." ".$_GET['post']['oder']."" ) );
if ( $_GET['delsearch'] == "yes" )
{
	query( "DELETE FROM simmoi ".$where );
}
$i = 0;
while ( $row = fecth( $sql ) )
{

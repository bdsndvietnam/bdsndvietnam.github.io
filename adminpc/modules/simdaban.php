...
class baogia
{
	...
	function xoaall($arr)
	{
		...
		{
			mysql_query("DELETE FROM daban WHERE id='".$value."'");
		}
	}
...
$where="WHERE sosim rlike'.*".$txtsim.".*'";
}
else 
{
$txtsim=str_replace(array('x','X','*'),array('[0-9]','[0-9]','.*'),$txtsim);
...
$sql=query("SELECT * FROM daban $where order by id desc $limit");
$dem=mysql_num_rows(query("SELECT * FROM daban $where"));
$stv->assign("mysql",urlencode("SELECT * FROM daban $where ".$_GET['post']['oder'].""));
